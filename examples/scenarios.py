...
def table_markdown() -> str:
    ...
    contract = load_contract(CONTRACT_PATH)
    rows = ["| Variant | Change | Expected | Status | Findings | Max severity |"]
    rows.append("| --- | --- | --- | --- | --- | --- |")
    cosmetic_flagged = 0
    ...
    for relative, change in TABLE_ROWS:
        ...
        rows.append(
            f"| `{relative}` | {change} | {expected} | {report.status} |"
            f" {len(report.findings)} | {severity} |"
        )
        if relative.startswith("v1c/") and report.status != "OK":
            cosmetic_flagged += 1
    ...
    rows.append(
        ...
        f" cosmetic variants flagged {cosmetic_flagged}/{cosmetic_total},"
