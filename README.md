...
`2026-01-01T00:00:00Z`, fixes the run id and seeds the jitter, so the journal is byte identical across runs, which is
what lets `tests/test_e2e.py` run all three and diff every line printed below against what came out.
...
filled, which is what makes the claim checkable rather than asserted. It is regenerated by
`uv run python scripts/capture_evidence.py --screenshot`, which records the boxes it painted into
`docs/evidence/screenshot.json`, and `tests/test_e2e.py` reads those pixels back out of the committed file and requires
the mask colour at the middle of every one. It reads them out of a live shot too, because an assertion about the mask
argument the driver was handed stays true whatever the browser then does. The demo credential hint line is masked too,
and a test walks the login page
...
breaker, the journal and the whole demo job through a `FakeDriver` in `tests/conftest.py`, which is why the 10 tests in
